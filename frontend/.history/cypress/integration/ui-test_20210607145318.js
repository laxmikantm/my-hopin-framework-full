    ...
    searchPage.submitBtn().click()
       ...
       let i=1;
            testdata.forEach(data => {    
              ...
              cy.xpath(`//table/tbody/tr[${i++}]`)
                .within(() => {
                    cy.get('td a[href="#"]').click({timeout: 1000})

               cy.location('pathname', {timeout: 1000})
                  .should('include', '/');
               cy.wait(200)   
      
                    // cy.xpath('//p[contains(text(),"Hi")]/following-sibling::div/p').contains(name)

                    // cy.get('input[type=button][value="Back to the list"]', {timeout:1000}).click()
                    cy.get('input', {timeout:1000}).click()
                    // cy.go('back')
