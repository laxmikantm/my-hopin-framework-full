...
describe("Basic Customer search", () =>{
  ...
  it("Able to search", function()  {
    ...
    cy.fixture('example2').its('name').toString()
    cy.get('tbody>tr>td> a[href="#"]:first-child').should('equals', cy.fixture('example2').its('name').toString())
    console.log('DEBUG, Output ->', cy.fixture('example').its('customers').its(0))
    console.log('DEBUG, Output ->', cy.fixture("myPeople").its("people").its(0).its("firstName"))
    cy.fixture("myPeople").its("people").then(people => people.map(p => p.firstName))
  })
