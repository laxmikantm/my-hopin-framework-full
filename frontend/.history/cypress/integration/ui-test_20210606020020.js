...
describe("Basic Customer search", () =>{
  ...
  it("Able to search", function()  {
    ...
    cy.get('tbody>tr>td> a[href="#"]').first().should('equals', this.data.name)

    cy.get('tbody>tr>td> a[href="#"]:first-child').within(($form) => {
            cy.get('a[href="#"]').first().should('equals', this.data.name)
        })
