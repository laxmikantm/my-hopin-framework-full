      ...
      searchPage.submitBtn().click()
       ...
       let i=1;
            testdata.forEach(data => {    
              ...
              const size = data.size
              cy.xpath(`//table/tbody/tr[${i++}]`)
                .within(() => cy.get('td a[href="#"]')
                                .click());       
                    cy.get('body')
                      .contains('Customer Details')   
                    resultsPage.getEmployeeName()
                      .contains(name)
                    ...
                    cy.xpath('//p[contains(text(),"Hi")]/following-sibling::div/p/b[contains(text(),"Size")]/parent::*')
                      .contains(size)
                    cy.xpath('//p[contains(text(),"Hi")]/following-sibling::div/p/b[contains(text(),"Contact")]/parent::*')
                      // .contains(JSON.parse(JSON.stringify(contactInfo)))                                              
